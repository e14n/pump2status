...
var UnavailableFeedError = function(url, wrapped) {
    Error.captureStackTrace(this, UnavailableFeedError);
    this.url = url;
    this.wrapped = wrapped;
};

UnavailableFeedError.prototype = new Error();  
UnavailableFeedError.prototype.constructor = UnavailableFeedError;
                        ...
                        if (err) {
                            ...
                            if (err instanceof UnavailableFeedError) {
                                removeUser(user, function(err) {
                                    if (err) {
                                        log.error({err: err, user: user.id}, "Error deleting user after failed feed");
                                    } else {
                                        log.info({user: user.id}, "Deleted user after failed feed");
                                    }
                                });
                            }
                        } else {
                ...
                getNewActivities = function(callback) {
                    ...
                    async.waterfall([
                        ...
                        function(host, callback) {
                            ...
                            oa.get(url, user.token, user.secret, function(err, data, response) {
                                if (err) {
                            ...
                            auto.forwardActivity(ppna, site, function(err) {
                                if (err) {
                                    ...
                                    if (err instanceof LinkError) {
                                        log.info({user: user.id, fuser: auto.id}, "Removing disconnected user");
                                        auto.del(function(err) {
                                            if (err) {
                                                log.error({user: user.id, fuser: auto.id, err: err}, "Error removing disconnected foreign user");
                                            } else {
                                                log.info({user: user.id, fuser: auto.id}, "Removed disconnected user");
                                            }
                                        });
                                    }
                                } else {
        ...
        removeUser = function(user, callback) {
            var a = 0,
                delShadow = function(shadow, callback) {
                    log.debug({shadow: shadow}, "Deleting shadow");
                    shadow.del(callback);
                },
                delEdge = function(edge, callback) {
                    log.debug({edge: edge}, "Deleting edge");
                    edge.del(callback);
                };
            async.parallel([
                function(callback) {
                    Shadow.search({pumpio: user.id}, function(err, shadows) {
                        if (err) {
                            callback(err);
                        } else {
                ...
                function(callback) {
                    Edge.search({from: user.id}, function(err, edges) {
                        if (err) {
                            callback(err);
                        } else {
                ...
                function(callback) {
                    Edge.search({to: user.id}, function(err, edges) {
                        if (err) {
                            callback(err);
                        } else {
                            async.each(edges, delEdge, callback);
