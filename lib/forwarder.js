            ...
            User.scan(
                ...
                function(err) {
                    if (err) {
                        ...
                        log.info({userCount: cnt}, "Done queueing users");
                    }
                }
            );
        },
            ...
            async.waterfall([
                ...
                function(results, callback) {
                    ...
                    if (_.isArray(autos) && autos.length > 0) {
                        log.info({autos: _.pluck(autos, "id"), user: user.id}, "Accounts updating automatically.");
                        getNewActivities(callback);
                    } else {
                        // We can skip costly requests if we're not forwarding the activities
                        log.info({user: user.id}, "No autos; skipping polling.");
                        callback(null, []);
                    }
                },
                function(results, callback) {
                    var ppnas = results;
                    log.info({ppnas: ppnas.length, user: user.id}, "Activities to forward.");
                        ...
                        async.each(autos, function(auto, callback) {
                            ...
                            auto.forwardActivity(ppna, site, function(err) {
                                if (err) {
                                    ...
                                    log.info({user: user.id, fuser: auto.id, activity: ppna.id}, "Forwarded");
                                }
                                // Ignore errors
                                callback(null);
                            });
